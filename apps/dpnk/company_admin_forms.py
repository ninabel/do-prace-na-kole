...
class SelectUsersPayForm(SubmitMixin, forms.Form):
    ...
    def __init__(self, request=None, *args, **kwargs):
        ...
        self.fields["paying_for"].widget.datatable_options = {
            "language": {
                "url": static(
                    "/js/datatables-plugins/i18n/%s.json" % request.LANGUAGE_CODE
                )
            },
        }
        return ret_val
...
class SubsidiaryForm(SubmitMixin, AddressForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["city"].queryset = City.objects.filter(
            cityincampaign__campaign=company_admin.campaign
        )
        self.helper.layout.extend(
            ["box_addressee_name", "box_addressee_telephone", "box_addressee_email"]
        )
        return ret_val
...
class CompanyAdminForm(SubmitMixin, UserAttendanceUpdateForm):
    motivation_company_admin = forms.CharField(
        ...
        help_text=_("Řekněte nám něco o pozici, kterou ve společnosti zastupujete."),
        max_length=100,
        required=True,
    )
    will_pay_opt_in = forms.BooleanField(
        label=_(
            "Zajistím, aby společnost uhradila startovné za náš zaměstnanecký tým."
        ),
        required=True,
    )
...
class CompanyCompetitionForm(SubmitMixin, forms.ModelForm):
    ...
    def clean_name(self):
        if not self.instance.pk:
            self.instance.slug = "FA-%s-%s" % (
                self.instance.campaign.pk,
                slugify(self.cleaned_data["name"])[0:30],
            )
