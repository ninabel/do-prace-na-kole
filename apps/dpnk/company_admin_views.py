...
from .company_admin_forms import (
    CompanyAdminApplicationForm,
    CompanyAdminForm,
    CompanyCompetitionForm,
    CompanyForm,
    SelectUsersPayForm,
    SubsidiaryForm,
)
from .email import (
    company_admin_register_competitor_mail,
    company_admin_register_no_competitor_mail,
)
from .models import (
    Campaign,
    Company,
...
class CompanyStructure(
    TitleViewMixin, MustBeCompanyAdminMixin, LoginRequiredMixin, TemplateView
):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        context_data["subsidiaries"] = context_data[
            "company"
        ].subsidiaries.prefetch_related(
        ...
        context_data["company_address"] = models.get_address_string(
            self.company_admin.administrated_company.address
        )
...
class RelatedCompetitionsView(
    MustBeCompanyAdminMixin, LoginRequiredMixin, TitleViewMixin, TemplateView
):
        ...
        for user_attendance in paying_for:
            for payment in user_attendance.payments().all():
                if payment.pay_type == "fc":
                    ...
                    break
        logger.info(
            "Company admin %s is paying for following users: %s"
            % (self.request.user, map(lambda x: x, paying_for))
        )
...
class CompanyAdminView(
        ...
        if old_company_admin.exists():
            return {
                "fullpage_error_message": _(
                    "Vaše organizce již svého koordinátora má: %s."
                )
                % (", ".join([str(c) for c in old_company_admin.all()])),
    ...
    def get_object(self, queryset=None):
        campaign = self.user_attendance.campaign
        try:
            self.company_admin = self.request.user.userprofile.company_admin.get(
                campaign=campaign
            )
        except CompanyAdmin.DoesNotExist:
            self.company_admin = CompanyAdmin(
                userprofile=self.request.user.userprofile, campaign=campaign
            )
        self.company_admin.administrated_company = (
            self.user_attendance.team.subsidiary.company
        )
...
class EditSubsidiaryView(
    ...
    def get_queryset(self):
        return (
            super()
            .get_queryset()
            .filter(company=self.company_admin.administrated_company)
        )
...
class CompanyCompetitionView(
    ...
    def get(self, *args, **kwargs):
        try:
            ...
            return render(
                self.request,
                self.template_name,
                context={"fullpage_error_message": e.args[0], "title": e.args[1]},
            )
    ...
    def get_object(self, queryset=None):
        ...
        if competition_slug:
            ...
            if (
                Competition.objects.filter(company=company, campaign=campaign).count()
...
class CompanyCompetitionsShowView(
    TitleViewMixin, MustBeCompanyAdminMixin, LoginRequiredMixin, TemplateView
):
...
class InvoicesView(
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        payments = models.payments_to_invoice(
            self.company_admin.administrated_company, self.company_admin.campaign
        )
